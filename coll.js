...
n => n - 1, n => n + 1, function mathMod(m, p) {
    if (arguments.length === 1)
        return _p => mathMod(m, _p);
    if (!_isInteger$1(m) || !_isInteger$1(p) || p < 1)
        return NaN;
    return (m % p + p) % p;
}, function mean(list) {
    ...
    if (arguments.length === 1)
        return _b => modulo(a, _b);
    return a % b;
}, function multiply(a, b) {
    if (arguments.length === 1)
        return _b => multiply(a, _b);
    return a * b;
}, function negate(n) {
    return -n;
    ...
    if (arguments.length === 1)
        return _list => find(fn, _list);
    return list.find(fn);
}, function findIndex(fn, list) {
    if (arguments.length === 1)
        return _list => findIndex(fn, _list);
    const len = list.length;
    let index = -1;
    while (++index < len) {
    ...
    if (arguments.length === 1)
        return _input => includes(target, _input);
    if (typeof list === 'string') {
    ...
    if (arguments.length === 1)
        return _list => indexOf(target, _list);
    let index = -1;
    const { length } = list;
    while (++index < length) {
    ...
    if (arguments.length === 1)
        return _list => intersection(list1, _list);
    ...
    if (arguments.length === 1)
        return _list => join(separator, _list);
    ...
    if (arguments.length === 1)
        return _list => lastIndexOf(target, _list);
    ...
    if (arguments.length === 1)
        return _list => none(fn, _list);
    return list.filter(fn).length === 0;
}, function nth(offset, list) {
    if (arguments.length === 1)
        return _list => nth(offset, _list);
    ...
    if (arguments.length === 1)
        return _list => reject(fn, _list);
    return filter((x, i) => !fn(x, i), list);
}, function repeat(val, n) {
    if (arguments.length === 1)
        return _n => repeat(val, _n);
    ...
    if (arguments.length === 1)
        return _list => sort(fn, _list);
    const arrClone = list.slice();
    return arrClone.sort(fn);
}, function splitEvery(n, list) {
    if (arguments.length === 1)
        return _list => splitEvery(n, _list);
    if (n < 1)
    ...
    if (arguments.length === 1)
        return _list => startsWith(prefix, _list);
    return list.startsWith(prefix);
}, function tail(list) {
    return drop(1, list);
}, function take(n, list) {
    if (arguments.length === 1)
        return _list => take(n, _list);
    if (n < 0)
        return list.slice();
    if (typeof list === 'string')
    ...
    if (arguments.length === 1)
        return _list => uniqWith(fn, _list);
    let index = -1;
    const len = list.length;
    const willReturn = [];
    while (++index < len) {
    ...
    if (arguments.length === 1)
        return _props => merge(obj, _props);
    return Object.assign({}, obj || {}, props || {});
}, function omit(keys, obj) {
    if (arguments.length === 1)
        return _obj => omit(keys, _obj);
    if (obj === null || obj === undefined) {
        return undefined;
    }
    ...
    if (arguments.length === 2)
        return _obj => propOr(defaultValue, p, _obj);
    if (arguments.length === 1)
        return (_p, _obj) => propOr(defaultValue, _p, _obj);
    if (!obj)
        return defaultValue;
